...
public class Main {
    ...
    static InitializedData initializedData;
    private static SecretKey serializedDirectoriesPasswordKey;


    static class InitializedData {
        final User user;
        final PrivateKey privateKey;
        final byte[] encodedPublicKey, signatureBytes;

        InitializedData(final User user, PrivateKey privateKey, byte[] encodedPublicKey, byte[] signatureBytes) {
    ...
    static {
        ...
        } catch (UnsatisfiedLinkError|NoSuchFieldException|IllegalAccessException ex) {
...
//        }
        ...
        // add a PKCS11 provider from config file
        try{
            if(SystemUtils.IS_OS_WINDOWS){
                Provider p = new SunPKCS11(new FileInputStream(new File(Constants.INSTALL_DIR, "cfgpkcs11_win")));
                Security.addProvider(p);
            }
            else if(SystemUtils.IS_OS_LINUX){
                Provider p = new SunPKCS11(new FileInputStream(new File(Constants.INSTALL_DIR, "cfgpkcs11_linux")));
                Security.addProvider(p);
            }
        }catch (IOException ex) {
            JOptionPane.showMessageDialog(null, "PKCS11 configuration file was not found in the installation directory.\n" +
                    "Please install the application again!",
                    "Protbox not supported", JOptionPane.ERROR_MESSAGE);
            System.exit(1);
        }
        ...
        SwingUtilities.invokeLater(new Runnable() {
            ...
            public void run() {
                if (Constants.verbose)
                    if (Constants.verbose) logger.info("Starting application");
                    ...
                    try {
                        if(serializedDirectoriesPasswordKey !=null){
                            ...
                            if(serializedDirectoryFiles.isEmpty() || list == null){
                                if(Constants.verbose) logger.info("No directory files were found: running app as first time!");
        ...
        try{
            for(PairReference<File, byte[]> pair : dataList) {
                ...
                if(!directory.accessUser.equals(initializedData.user)){
                    ...
                    if(Constants.verbose) logger.info("Added directory "+directory.NAME +" to instance list...");
                }
            }
    ...
    private static void exit() {
        ...
        if(initializedData !=null && trayApplet!=null && trayApplet.instanceList!=null && trayApplet.instanceList.getComponents().length!=0){
            if(Constants.verbose) logger.info("Serializing and saving directories...");
    ...
    static void showTrayApplet() throws AWTException {
        tray.add(trayApplet.trayIcon);
    }

    static void hideTrayApplet() {
